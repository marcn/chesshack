...
def main(argv):
    filename1 = argv[0]
    board1 = cv2.imread(filename1, cv2.IMREAD_COLOR)
    (board1b, board1g, board1r) = cv2.split(board1)

    cv2.imshow("game board r" , board1r)
    cv2.imshow("game board g" , board1g)
    cv2.imshow("game board b" , board1b)


    #val = np.mean(abs(board1.astype(np.float) - board2.astype(np.float)))
    val1 = np.mean(abs(board1r.astype(np.float)))
    val2 = np.sum(abs(board1r.astype(np.float)))
    print val1, val2
    cv2.waitKey(0);
